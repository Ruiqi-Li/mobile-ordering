 ...
 */
public abstract class ArrayIndexerListAdapter extends BaseAdapter implements
		SectionIndexer {
	...
	public ArrayIndexerListAdapter(Context context) {
		mContext = context;
	}
	 ...
	 * as the user scrolls.
	 */
	protected abstract View createPinnedSectionHeaderView(Context context,
			ViewGroup parent);

	/**
	 * Sets the title in the pinned header as the user scrolls.
	 */
	protected abstract void setPinnedSectionTitle(View pinnedHeaderView,
			String title);
	...
	/**
	 * @param position
	 *            relative position in the indexed partition
	 */
	...
	public View getPinnedHeaderView(int viewIndex, View convertView,
			ViewGroup parent) {
		if (isSectionHeaderDisplayEnabled()
	...
	public void configurePinnedHeaders(PinnedHeaderListView listView) {
		...
		if (mIndexer == null || getCount() == 0) {
			...
			int listPosition = listView.getPositionAt(listView
			...
			if (section == -1) {
				...
				setPinnedSectionTitle(mHeader,
						(String) mIndexer.getSections()[section]);
				// setPinnedHeaderContactsCount(mHeader,
				// mIndexer.getSections()[section]);
				...
				// Log.d("INDEX", "setFadingHeader    index = " + index +
				// " ; listPosition = " + listPosition + "; isLastInSection = "
				// + isLastInSection);
			}
		}
	}
	 ...
	 * {@code placement} object accordingly. Please note that the returned
	 * object is volatile and should be copied if the result needs to be used
	 * later.
	 */
